...
TYPES = {
    "RiskAssessment": [
        {"name": "intentId", "type": "bytes32"}, {"name": "canBeInsured", "type": "bool"},
        {"name": "tradeAllowed", "type": "bool"}, {"name": "riskScore", "type": "uint256"},
        {"name": "premium", "type": "uint256"}, {"name": "coverageAmount", "type": "uint256"},
...
def sign(intent_id: str, decision: UnsignedDecision, private_key: str, chain_id: int, verifying_contract: str) -> RiskReport:
    ...
    message = {"intentId": intent_id, "canBeInsured": decision.can_be_insured, "tradeAllowed": decision.trade_allowed,
               "riskScore": decision.risk_score, "premium": decision.premium, "coverageAmount": decision.coverage_amount,
               "coverageDuration": decision.coverage_duration, "assessedAt": int(assessed_at.timestamp()), "expiresAt": int(expires_at.timestamp())}
    ...
    return RiskReport(intentId=intent_id, canBeInsured=decision.can_be_insured, tradeAllowed=decision.trade_allowed,
                      riskScore=decision.risk_score, premium=decision.premium, coverageAmount=decision.coverage_amount,
                      coverageDuration=decision.coverage_duration, signature=signed.signature.hex(), assessedAt=assessed_at,
